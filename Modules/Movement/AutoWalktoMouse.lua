...
registerModule("Movement", "Auto-Walk to Mouse", 300, 50, true, S.PathfindingWalk, function(v)
    ...
    if v then
        task.spawn(function()
            while S.PathfindingWalk do
                local hum = getHRP(); local hrp = getHRP(); if not hum or not hrp then task.wait(0.5); continue end
                ...
                if ok and path.Status == Enum.PathStatus.Success then
                    ...
                    for i, wp in ipairs(waypoints) do
                        if not S.PathfindingWalk then break end
                        if wp.Action == Enum.PathWaypointAction.Jump then hum.Jump = true end
                        hum:MoveTo(wp.Position); local timeout = tick() + 1
                        repeat task.wait() if (hrp.Position - wp.Position).Magnitude < 4 or tick() > timeout then break end until not S.PathfindingWalk
                    end
                else hum:MoveTo(targetPos); task.wait(0.5) end
                task.wait(0.1)
            end
        end)
    else local hum = getHum(); if hum then hum:MoveTo(getHRP().Position) end end
    saveConfig()
end)
